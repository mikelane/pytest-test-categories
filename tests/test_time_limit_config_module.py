...
@pytest.mark.small
class DescribeTimeLimitConfigFromDict:
    """Test suite for creating TimeLimitConfig from dictionary."""

    def it_creates_from_full_dict(self) -> None:
        """Create config from dictionary with all values."""
        from pytest_test_categories.timing import TimeLimitConfig
        ...
        config = TimeLimitConfig.from_dict(data)
    ...
    def it_creates_from_partial_dict(self) -> None:
        """Create config from dictionary with only some values, using defaults for rest."""
        from pytest_test_categories.timing import TimeLimitConfig
        ...
        assert config.small == 2.0
        assert config.medium == 300.0  # default
        assert config.large == 900.0  # default
        assert config.xlarge == 900.0  # default

    def it_creates_from_empty_dict(self) -> None:
        """Create config from empty dictionary uses all defaults."""
        from pytest_test_categories.timing import TimeLimitConfig

        config = TimeLimitConfig.from_dict({})
    ...
    def it_handles_string_values(self) -> None:
        """Create config from dictionary with string values (as from ini files)."""
        from pytest_test_categories.timing import TimeLimitConfig
    ...
    def it_handles_integer_values(self) -> None:
        """Create config from dictionary with integer values."""
        from pytest_test_categories.timing import TimeLimitConfig
    ...
    def it_ignores_unknown_keys(self) -> None:
        """Ignore unknown keys in dictionary."""
        from pytest_test_categories.timing import TimeLimitConfig
